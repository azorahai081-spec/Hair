...
$shipping_value = $_POST['shipping'] ?? '70'; // Default to 70 if not set
...
// Determine shipping cost and location text based on the value.
$shipping_cost = ($shipping_value === '120') ? 120 : 70;
$shipping_location = ($shipping_value === '120') ? 'ঢাকার বাইরে:' : 'ঢাকার মধ্যে:';
$product_price = 1490;
$total_price = $product_price + $shipping_cost;

// --- FIX: Explicitly set the current date and time ---
...
try {
    ...
    $stmt = $pdo->prepare(
        "INSERT INTO orders (customer_name, customer_address, customer_phone, product_name, shipping_location, shipping_cost, total_price, order_number, order_date) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );
    ...
    $_SESSION['form_error'] = "A database error occurred. Please try again.";
    header('Location: index.php#order-form');
    exit;
}
...
if ($last_insert_id > 0) {
    ...
    $_SESSION['form_error'] = "Could not save the order. Please try again.";
    header('Location: index.php#order-form');
    exit;
}
